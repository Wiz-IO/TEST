...
#define H_OFFSET            0 // 25
#define V_OFFSET            30

extern uint8_t frame_buffer[192][280 * 4 + 1]; // pixel * 4 + 1 column blank
...
static volatile unsigned int v_line = 0, y_line;
...
void __attribute__((vector(_OUTPUT_COMPARE_1_VECTOR), interrupt(IPL6SRS), nomips16)) OC1Handler(void)
{
    ...
    DCH0SSA = KVA_TO_PA(&frame_buffer[y_line]); // init DMA line
...
void vga_init()
{
    ...
    OC2RS = OC1R + VGA_HBP + 100;                         // DMA start + H offset

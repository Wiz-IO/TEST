...
OBJECT_TYPES = {
    'bytecode': 1,  # Главен код или функция
    'int': 2,
    'float': 3,     # Променяме на 3 за съответствие с MicroPython
    'string': 4,    # Променяме на 4
    'bytes': 5,     # Нов тип за бинарни данни
    'list': 6,      # Променяме на 6
    'tuple': 7,     # Отделен тип за кортежи
    'dict': 8,
    'bool': 9,
    'none': 10,
    'function': 11, # Отделен тип за функции
    'name': 12,     # Променяме на 12
    'varname': 13,  # Променяме на 13
    'class': 14,    # Нов тип за класове
    'instance': 15, # Нов тип за инстанции (ако са статични)
    'module': 16,   # Нов тип за модули
    'builtin': 17   # Променяме на 17
}
...
class BinaryGenerator:
    ...
    def process_class(self, code_obj, module_id):
        """Обработва клас (име + методи + parent)."""
        ...
        if class_name:
            ...
            data.extend(struct.pack('<I', name_idx))  # Име на класа
            method_count = 0
            for const in code_obj.co_consts:
                if isinstance(const, types.CodeType):
                    ...
                    method_count += 1
            data = struct.pack('<H', method_count) + data  # Брой методи преди данните
    ...
    def process_code(self, code_obj, module_id):
        ...
        for const in code_obj.co_consts:
            if isinstance(const, types.CodeType):
                # Проверяваме дали е клас (LOAD_BUILD_CLASS в байткода)
                instructions = list(dis.get_instructions(code_obj))
